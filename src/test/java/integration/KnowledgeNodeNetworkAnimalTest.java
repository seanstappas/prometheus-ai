...
public class KnowledgeNodeNetworkAnimalTest {
  ...
  public void forwardSearchTest() {
    ...
    expectedActiveTagsAndBeliefs
        .put(new Fact("straightforward(smart,precise)"), 50.0);
    ...
    expectedActiveTagsAndBeliefs.put(new Rule(
            "bark(sound,loud) pet(scary,attractive) -> @isPet(easy,calm,bark)"),
        72.0);
    expectedActiveTagsAndBeliefs.put(new Fact("fast(speed,dynamic)"), 50.0);
    expectedActiveTagsAndBeliefs.put(new Rule(
            "cat(feline,length>50,weight>20) dog(wolflike,length>50,weight>20) -> @fight(dangerous)"),
        100.0);
    expectedActiveTagsAndBeliefs
        .put(new Fact("transport(easy,speed>10)"), 22.0);
    expectedActiveTagsAndBeliefs.put(new Fact("fur(strands,insulator)"), 100.0);
    expectedActiveTagsAndBeliefs
        .put(new Fact("mammal(vertebrate,land)"), 68.75);
    ...
    expectedActiveTagsAndBeliefs.put(new Rule(
            "massive(big,heavy) teeth(grind,food) -> @scary(scary,dangerous)"),
        100.0);
    expectedActiveTagsAndBeliefs
        .put(new Rule("enemy(scary,dangerous) -> @avoid(scary,dangerous)"),
            40.5);
    expectedActiveTagsAndBeliefs.put(
        new Rule("coward(scared,safe) loyal(easy,calm) -> @isSafe(easy,calm)"),
        24.2);

    System.out
        .println("[KNN] Active tags after foward searching: " + activeTags);
  ...
  public void forwardSearchWithPlyTest() {
    ...
    System.out.println(
        "[KNN] Active tags after 0 ply forward searching: " + activeTags);
  ...
  public void backwardSearchTest() {
    ...
    expectedActiveTagsAndBeliefs
        .put(new Fact("feathers(floating,insulator,flight)"), 30.0);
    expectedActiveTagsAndBeliefs
        .put(new Fact("dog(wolflike,length>50,weight>20)"), 60.0);
    ...
    expectedActiveTagsAndBeliefs
        .put(new Fact("cat(feline,length>50,weight>20)"), 80.0);

    System.out
        .println("[KNN] Active tags after backward searching: " + activeTags);
  ...
  public void backwardSearchWithPlyTest() {
    ...
    System.out.println(
        "[KNN] Active tags after 2 ply backward searching: " + activeTags);
  ...
  public void lambdaSearchTest() {
    ...
    expectedActiveTagsAndBeliefs.put(new Fact("fast(speed,dynamic)"), 100.0);
    expectedActiveTagsAndBeliefs
        .put(new Fact("dog(wolflike,length>50,weight>20)"), 100.0);

    System.out.println("[KNN] String trying to find out a link with: " + item);
    System.out
        .println("[KNN] Active tags after lambda searching: " + activeTags);
